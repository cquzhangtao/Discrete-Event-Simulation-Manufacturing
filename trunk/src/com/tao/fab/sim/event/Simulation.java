...
import simulation.core.SimulationListener;
...
public class Simulation extends Entity {
	...
	private SimulationEventList evnetLists;
	
	public SimulationEventList getEvnetLists() {
		return evnetLists;
	}

	public Simulation(){
		evnetLists=new SimulationEventList(this,eventList);
	}
	...
	public void run(){
		...
		while(running&&!eventList.isEmpty()&&!AreAllPeriodicalEvents()&&!meetEndCondition()){
			...
			event.response(evnetLists,currentTime);
	...
	public void clone(Simulation simulation){
